...
#MODEL
def setup_wandb(run_id="Run_overfit-1-mtp_mp_gqa_ddp",entity="rohit_iisc-indian-institute-of-science",project_id="llm_overfit"):
    run_id=run_id
    run = wandb.init(
        entity=entity,
        project=project_id,
...
def load_model(run,device):
    ...
    if run is not None:   #FIX1: run is None on non-zero ranks
...
def train_step(transformer_model,x,d_model,embed_weight,ce_main,ce_mtp,lse_square_scale=1e-4,mtp_k=0,mtp_weight=0): #CE EXPECTS LIGER FUSED KERNELS
    B,T=x.shape
...
def save_training_checkpoint(master_param_1d,master_param_2d,
                             muon_optim_fp32,adamw_optim_fp32, step, path="Weights/checkpoint.pt"):
    checkpoint = {
        'step': step,
...
def train():
    ...
    run=setup_wandb() if local_rank==0 else None

    dataloader=load_dataset(local_rank)
    ...
    with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
        ...
        for global_step in tqdm(range(cfg.TOTAL_STEPS)):
            ...
            fwd_start=time.time()
            loss,logsumexp_avg=train_step(optimized_model,x,embed_weight=embed_weight,d_model=cfg.D_MODEL,ce_main=liger_fused_ce_main,
                                          ce_mtp=liger_fused_ce_mtp,mtp_weight=cfg.MTP_LOSS_WEIGHT,mtp_k=cfg.MTP_HEADS)


            #CONTEXT MANAGEMENT TO FIRE DDP ONLY WHEN ACCUMULATION STEP ARRIVES FOR OPTIM UPDATE
            loss_for_log=loss.detach()   #FIX3: keep UNSCALED loss for logging
            ...
            if is_sync_step:
                ...
                if is_log_step and run:
                    prev=wl.snapshot_params(transformer_model.module)   #FIX4: fold shard_ids into single run.log below
                ...
                act_stats={}
                if is_log_step and run:
                    save_model(transformer_model.module)
                    act_stats=wl.activation_stats(transformer_model.module,x)   #FIX2: no ddp kwarg
                ...
                if run:
                    loss_train=loss_for_log.item()   #FIX3: unscaled loss

                    f_time=time.time()
                    tokens_seen+=cfg.BATCH_SIZE*cfg.EFF_SEQ_LEN*cfg.ACCUMULATION_STEP*dist.get_world_size()  #FIX6: global tokens
                    ...
                    logs.update({"loss/ema_ce":ema_loss_log,"perf/ema_throughput":ema_throughput_log,
                                "loss/logsumexp_sq":logsumexp_avg,"data/shard_ids":shard_ids})  #FIX4: shard_ids folded in

...
def precompute_rope_fast(max_context,max_freq,head_dim): # USES OUTER PRODUCT TO BUILD THETA VALUES AND APPLY SIN COS AT ONCE
    ...
    i=torch.arange(0,head_dim)
    i=(i-i%2)/head_dim
    
    theta=1/torch.pow(max_freq,i)
    pos_theta=torch.outer(pos_tensor,theta)
    return torch.cos(pos_theta),torch.sin(pos_theta)
...
def apply_rope(x,cos_embed,sin_embed,perm,alt_bit): #EITHER COMPUTE ROPE ONCE FOR MAX CONTEXT OR COMPUTE FOR EACH CONTEXT LENGTH PER BATCH CHOICE 
    T=x.shape[-2]

    cos,sin=cos_embed[:T,:].to(x.dtype),sin_embed[:T,:].to(x.dtype)
    x_perm = x[..., perm] * alt_bit
    return x*cos + x_perm*sin
...
class mhma(nn.Module):
    ...
    def forward(self,x,mask,cos,sin,perm,alt_bit):
        B,T,D=x.shape
        residual=x
        ...
        q,k=apply_rope(q,cos,sin,perm,alt_bit),apply_rope(k,cos,sin,perm,alt_bit)
...
class transformer_block(nn.Module):
    ...
    def forward(self,x,mask,cos,sin,perm,alt_bit):
        x=self.att(x,mask,cos,sin,perm,alt_bit)
        residual=x
...
class Transformer(nn.Module):
    def __init__(self,vocab_size,max_context,max_freq,d_model,n_heads,num_layers,attn_dropout,ffn_hidden_dim,ffn_dropout):
        ...
        self.register_buffer("alt_bit",alt_bit)
    ...
    def forward(self,x):
        ...
        for trans_block in self.transformer_block_list:
            x=trans_block(x,self.mask[:T,:T],self.cos[:T],self.sin[:T],self.perm,self.alt_bit)
...
x=att(x,mask,rope_cos,rope_sin,perm,alt_bit)
print(x.shape)

x=torch.arange(2048)
x=torch.stack((x,x,x),dim=0).to("cuda:0")
# x=x.to(torch.bfloat16)

trans=Transformer(50000,2048,10000,2048,32,14,0.1,8192,0.1)
trans=trans.to(torch.bfloat16).to("cuda:0")
x=trans(x)
print(x.shape)


# perm,alt_bit=permute_indcies_rope(6)
# x=torch.randn(2,2,2,6)
# print(alt_bit)
# print(x[:, :,:,perm]*alt_bit,x)

# linear=linear_fc(4,2,"cuda:6")
# tensor=torch.randn((5,4),device="cuda:6")
# print(linear.weight.shape)
# print(linear(tensor).shape)


# layer=layer_norm(5).to("cuda:6")
# tensor=torch.randn((3,5),device="cuda:6")
# print(layer(tensor).shape)
# print(layer(tensor).std(dim=1))

# rms=rms_norm(5).to("cuda:6")
# tensor=torch.randn((3,5),device="cuda:6")
# print(rms(tensor).shape)
# print(rms(tensor).std(dim=1))